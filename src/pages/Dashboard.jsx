...
import { Thermometer, Zap, Activity, Gauge, FlaskConical, Wifi, Clock, RefreshCw, BarChart2, Battery } from 'lucide-react';

function MetricCard({ icon: Icon, label, value, unit, color = '#00daf3', alert }) {
  return (
    <div className={`bg-[#1c2026] rounded-xl p-4 flex flex-col gap-2 transition-all`}>
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <Icon size={14} style={{ color }}/>
          <span className="text-[9px] uppercase tracking-[0.15em] text-[#849396]">{label}</span>
        </div>
        {alert && <span className="text-[9px] text-[#ffb4ab] uppercase tracking-widest font-bold">HIGH</span>}
      </div>
    ...
    <div className="bg-[#181c22] rounded-xl p-5 border border-[#3b494c]/20">
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <div className="bg-[#10141a] rounded-lg p-3 text-center">
          ...
          <div className="text-lg font-black font-headline" style={{ color: barColor }}>{timeLeft ?? '--'}</div>
...
export default function Dashboard() {
  ...
  const tempAlert = r && r.temperature_c       > (s.tempLimit    || 85);
  const vibAlert  = r && r.vibration_rms_mm_s2 > (s.vibLimit     || 30);
  const currAlert = r && r.spindle_current_a   > (s.currentLimit || 42);
  const voltAlert = r && r.supply_voltage_v != null && r.supply_voltage_v < 4.8 && r.supply_voltage_v > 0;

  return (
    <div className={`p-6 space-y-6 transition-all`}>
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => setSimMode(m => !m)}
            className={`flex items-center gap-1.5 text-xs px-3 py-2 rounded-lg border font-bold transition-all
              ${simMode
                ? 'bg-[#ffba38]/10 border-[#ffba38]/40 text-[#ffba38]'
                : 'bg-[#3b494c]/10 border-[#3b494c]/30 text-[#849396] hover:text-[#dfe2eb]'}`}>
      ...
      {simMode && (
        <div className="bg-[#1a1a10] border border-[#ffba38]/30 rounded-xl p-5">
          <div className="text-[10px] uppercase tracking-[0.2em] text-[#ffba38] mb-4 font-bold">🧪 Simulation Mode</div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[['Temperature (°C)',simTemp,setSimTemp,'#ff9259','0','150','1'],
              ['Vibration (m/s²)',simVib, setSimVib, '#00e5ff','0','20', '0.1'],
              ['Current (A)',     simCurr,setSimCurr,'#818cf8','0','60', '0.1'],
              ['Voltage (V)',     simVolt,setSimVolt,'#4ade80','0','12', '0.1'],
            ].map(([label,val,setter,color,min,max,step]) => (
              <div key={label}>
                ...
                <input value={val} onChange={e=>setter(e.target.value)} type="number" min={min} max={max} step={step}
                  className="w-full bg-[#10141a] rounded-lg px-3 py-2.5 text-sm font-mono text-[#dfe2eb] outline-none border"
                  style={{borderColor:color+'40'}}/>
              </div>
            ))}
          </div>
          <div className="mt-3 flex gap-2 flex-wrap">
            {[{l:'Idle',t:'27',v:'0.9',c:'0.1',u:'4.95'},{l:'Light Cut',t:'45',v:'2.5',c:'8',u:'4.90'},
              {l:'Normal',t:'65',v:'3.5',c:'18',u:'4.85'},{l:'Heavy',t:'80',v:'6.0',c:'35',u:'4.75'},{l:'Critical',t:'90',v:'9.0',c:'45',u:'4.60'}
            ].map(p=>(
              <button key={p.l} onClick={()=>{setSimTemp(p.t);setSimVib(p.v);setSimCurr(p.c);setSimVolt(p.u);}}
      ...
      <div className="bg-[#181c22] rounded-xl p-4 border border-[#3b494c]/20 flex items-center gap-4">
        ...
        <div>
          ...
          <div className="text-2xl font-black font-headline text-[#818cf8] mt-0.5">
            {r?.dominant_freq_hz != null ? `${r.dominant_freq_hz.toFixed(3)} Hz` : '—'}
          </div>
          <div className="text-[9px] text-[#849396] mt-0.5">
            {r?.dominant_freq_hz == null ? 'Collecting samples…'
              : r.dominant_freq_hz < 0.05 ? 'Low freq — smooth cut'
              : r.dominant_freq_hz < 0.15 ? 'Mid freq — normal wear'
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
        ...
        <div className="bg-[#1c2026] rounded-xl p-4">
          <LiveChart data={chartData} dataKey="vibration_rms_mm_s2" color="#00e5ff" label="Vibration m/s²"  limit={s.vibLimit}/>
        </div>
        <div className="bg-[#1c2026] rounded-xl p-4">
          <LiveChart data={chartData} dataKey="spindle_current_a"   color="#818cf8" label="Current A"       limit={s.currentLimit}/>
        </div>
        <div className="bg-[#1c2026] rounded-xl p-4">
          <LiveChart data={chartData} dataKey="supply_voltage_v"    color="#4ade80" label="Voltage V"/>
        </div>
      </div>
    </div>
  );
